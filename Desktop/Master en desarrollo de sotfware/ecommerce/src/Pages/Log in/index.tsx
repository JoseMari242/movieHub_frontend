...
async function loginUser(email: string, password: string, users: User[], onLoginSuccess: () => void, navigate: any, dispatch: any) {
  const user = users.find(user => user.email === email && user.password === password);
  if (user) {
    dispatch({type:"LOGIN"});
    navigate("/home");
...
const Login: React.FC<Props> = ({ onLoginSuccess }) => {
  ...
  const handleLoginClick = async (event: React.FormEvent) => {
    event.preventDefault(); // Evita que se recargue la página al enviar el formulario
    await loginUser(email, password, users, onLoginSuccess, navigate, dispatch);
  };
